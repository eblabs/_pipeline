...
# this limb should do the ik Spring Solver rig functions
#####################################################
## import
...
class baseIkSpringSolverLimb(baseJointsLimb.baseJointsLimb):
	...
	def createComponent(self):
		super(baseIkSpringSolverLimb, self).createComponent()


		sParent_ctrl = self._sComponentControls

		## put ik joint chain locally
		sGrp_ikJnts = transforms.createTransformNode(naming.oName(sType = 'group', sSide = self._sSide, sPart = '%sSpringJointsLocal' %self._sName, iIndex = self._iIndex).sName, lLockHideAttrs = ['tx', 'ty', 'tz', 'rx', 'ry', 'rz', 'sx', 'sy', 'sz', 'v'], sParent = self._sComponentRigNodesWorld)
		sParent_jntLocal = sGrp_ikJnts
		lJntsLocal = []

		lJntsLocal, lBindJnts = createDriveJoints.createDriveJoints(self._lBpJnts, sParent = sGrp_ikJnts, sSuffix = 'IkSpringLocal', bBind = False)
		lJnts, lBindJnts = createDriveJoints.createDriveJoints(self._lBpJnts, sParent = self._sComponentJoints, sSuffix = 'IkSpring', bBind = self._bBind)
		...
		self._writeGeneralComponentInfo('baseIkSpringSolverLimb', lJnts, lCtrls, lBindJnts, self._lBindRootJnts)
