...
class MasterNode(object):
	...
	def create(self):
		# create groups
		attrDict = {}
		for grp in ['master', 'deformationRig', 'animationRig', 'geometry',
					'skeleton', 'rigNodes', 'control', 'components']:
		...
		cmds.parent(self._geometry, self._skeleton, self._rigNodes, self._deformationRig)
		cmds.parent(self._control, self._components, self._animationRig)
		...
		# connect attrs
		cmds.setAttr('{}.overrideEnabled'.format(self._geometry), 1)
		attributes.connectAttrs(['geometryVis', 'skeletonVis', 'rigNodesVis', 'geometryType'],
								['{}.v'.format(self._geometry), '{}.v'.format(self._skeleton),
								 '{}.v'.format(self._rigNodes), '{}.overrideDisplayTyp'.format(self._geometry)],
								 driver = self._deformationRig, force = True)

		# add master control
		ControlWorld = controls.create('world', side = 'middle', parent = self._control, lockHide = ['sx', 'sy', 'sz'],
